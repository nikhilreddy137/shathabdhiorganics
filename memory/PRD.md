...
- [2026-06] Announcement bar updated per user: removed "Free shipping over ₹500 / Shop now", now reads "Trusted by 20,000+ families · Stocked in 700+ stores across India" with gold-highlighted numbers (Header.jsx, data-testid announcement-bar). Verified via screenshot.
- [2026-06] "Shop the full collection" (Home) upgraded per user: now a bold soil-brown rounded pill CTA; it carries the active category chip filter to the internal catalog — e.g. with "Millets" chip selected it navigates to /collections/millets (filter pre-applied), default goes to /collections/best-sellers. E2E verified via screenshot click-through.
- [2026-06] Trust stats strip added on Home below marquee (trust-stats-strip): 20,000+ families · 700+ retail stores (B2B) · 2,400+ women farmers · Manikonda flagship store, serif italic soil numerals. Verified via screenshot.
- [2026-06] "Visit us in Manikonda" store section (components/StoreVisit.jsx) added on BOTH Home (after testimonials) and About (bottom): embedded Instagram store-opening reel (instagram.com/p/DOGmQs3D29W via /embed/ iframe — loads and shows the real reel), full address (2-35/177/1/A, Road #10, Neknampur, Puppalguda, Gandipet, Hyderabad 500089), Get Directions (Google Maps link) + Watch the Opening buttons. Verified on both pages via screenshot.
- [2026-06] Social page: added "Our latest reels, live from Instagram" section (social-reels-section) with 3 REAL embedded reels from @shathabdhiorganics via /p/{code}/embed/ iframes: DOGmQs3D29W (store opening), DbVGf1wPp1Y, DbIuSv8BHO9 (shortcodes extracted from the live profile embed HTML). Verified via screenshot — all 3 render with play buttons and like counts. Note: more shortcodes discoverable via curl https://www.instagram.com/shathabdhiorganics/embed/ .
- [2026-06] BUG FIX (user-reported empty images on /social): 4 hardcoded Shopify CDN image URLs in Social.jsx reels array were 404 (stale ?v= versions) — Urad Dal, Brown Rice, Pearl Millet, Butterfly Pea Tea tiles. Replaced with current live CDN URLs from the catalog. Testing agent iteration_4: 100% pass, all 12 tiles verified naturalWidth > 0, no broken images anywhere on /social.
